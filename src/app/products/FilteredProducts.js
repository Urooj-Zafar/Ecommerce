...
export default function Products() {
  ...
  // Separate filtered vs remaining products
  const filteredOnly = categoryFilter
  ...
  const remainingProducts = categoryFilter
    ? filteredProducts.filter(
        (p) =>
  ...
  // Combine both sections for pagination
  const combinedProducts = categoryFilter
    ? [...filteredOnly, ...remainingProducts]
    : filteredProducts;
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      {categoryFilter && filteredOnly.length > 0 && (
        <div className="mb-8">
          <h2 className="text-2xl font-bold mb-4">
            {filteredOnly[0].category?.title || "Selected Category"} Products
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
            {renderProducts(filteredOnly)}
          </div>
        </div>
      )}
